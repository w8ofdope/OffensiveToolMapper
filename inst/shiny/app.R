...
pipeline_snapshot_rows_by_run <- function(table_name, db_path) {
  empty_counts <- tibble::tibble(run_id = character(), count = integer())
  snapshots <- read_duckdb_table("pipeline_storage_snapshots", db_path = db_path, required = FALSE, restore = FALSE)

  if (!is.data.frame(snapshots) || nrow(snapshots) == 0) {
    return(empty_counts)
  }

  required_columns <- c("run_id", "table_name", "row_count", "stored_at")
  if (!all(required_columns %in% names(snapshots))) {
    return(empty_counts)
  }

  snapshots <- snapshots[
    !is.na(snapshots[["run_id"]]) &
      nzchar(as.character(snapshots[["run_id"]])) &
      snapshots[["table_name"]] == table_name,
    ,
    drop = FALSE
  ]
  ...
  snapshots[["stored_at_parsed"]] <- pipeline_parse_datetime(snapshots[["stored_at"]])
  snapshots[["row_order"]] <- seq_len(nrow(snapshots))
  snapshots <- snapshots[
    order(snapshots[["run_id"]], snapshots[["stored_at_parsed"]], snapshots[["row_order"]]),
    ,
    drop = FALSE
  ]

  latest <- snapshots[!duplicated(snapshots[["run_id"]], fromLast = TRUE), , drop = FALSE]
  tibble::tibble(
    run_id = as.character(latest[["run_id"]]),
    count = as.integer(latest[["row_count"]])
  )
}
...
build_pipeline_run_catalog <- function(db_path) {
  ...
  relevant_counts <- pipeline_snapshot_rows_by_run("relevant_tools", db_path)
  mitre_counts <- pipeline_snapshot_rows_by_run("mitre_mappings", db_path)
...
build_pipeline_snapshot <- function(data_dir) {
  ...
  db_path <- file.path(data_dir, "offensive_tool_mapper.duckdb")
  run_catalog <- build_pipeline_run_catalog(db_path)
  last_run <- if (is.data.frame(run_catalog) && nrow(run_catalog) > 0) run_catalog[1, , drop = FALSE] else empty_pipeline_run_catalog()
  ...
  metrics <- list(
    ...
    last_run_relevant_tools = if (nrow(last_run) > 0) as.integer(last_run$relevant_tools[[1]] %||% NA_integer_) else NA_integer_,
    last_run_discovery_new_rows = if (nrow(last_run) > 0) as.integer(last_run$discovery_new_rows[[1]] %||% NA_integer_) else NA_integer_,
    last_run_github_new_rows = if (nrow(last_run) > 0) as.integer(last_run$github_new_rows[[1]] %||% NA_integer_) else NA_integer_,
  ...
  list(
    ...
    duckdb = pipeline_duckdb_summary(db_path),
    duckdb_path = db_path,
...
app_server <- function(input, output, session) {
  ...
  output$modern_overview_panel <- shiny::renderUI({
    ...
    tags$div(
      ...
      tags$section(
        ...
        modern_section_heading("Лучшие утилиты", "Короткий срез лучших записей из текущего слоя", class = "heading-dark"),
  ...
  output$pipeline_metric_grid <- shiny::renderUI({
    ...
    tags$div(
      ...
      metric_card("Новых найдено", snapshot$metrics$last_run_discovery_new_rows %||% "n/a", "Новые записи из последнего сбора данных"),
      metric_card("Новых с GitHub", snapshot$metrics$last_run_github_new_rows %||% "n/a", "Новые репозитории GitHub за последний запуск"),
  ...
  shiny::observeEvent(input$pipeline_run_btn, {
    shiny::showModal(shiny::modalDialog(
      ...
      shiny::selectInput(
        ...
        selected = get_runtime_env_value("LLM_PROVIDER", unset = "openai")
      ),
      shiny::textInput(
        "modal_model",
        "Модель (пусто = по умолчанию)",
        value = get_runtime_env_value("LLM_MODEL", unset = ""),
      ...
      shiny::radioButtons(
        ...
        selected = get_runtime_env_value("OTM_COLLECT_MODE", unset = "incremental")
      ),
      shiny::numericInput(
        "modal_batch_count",
  ...
  shiny::observeEvent(input$modal_pipeline_confirm, {
    provider     <- input$modal_provider %||% get_runtime_env_value("LLM_PROVIDER", unset = "openai")
