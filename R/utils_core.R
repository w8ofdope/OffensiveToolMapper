...
read_dotenv_value <- function(name, path = getOption("offensivetoolmapper.dotenv_path", ".env")) {
  entry <- read_dotenv_entry(name, path)
  if (entry$found) entry$value else ""
}

#' Read a variable entry from .env
#'
#' @param name Variable name.
#' @param path Path to the dotenv file.
#'
#' @return List with `found` and `value`.
read_dotenv_entry <- function(name, path = getOption("offensivetoolmapper.dotenv_path", ".env")) {
  if (!file.exists(path)) {
    return(list(found = FALSE, value = ""))
  }
  ...
  for (line in lines) {
    ...
    key <- stringr::str_trim(substr(line, 1L, separator - 1L))
    if (!identical(key, name)) {
      next
    }

    value <- substr(line, separator + 1L, nchar(line))
    value <- stringr::str_trim(value)
    value <- gsub('^"|"$', "", value)
    value <- gsub("^'|'$", "", value)
    return(list(found = TRUE, value = value))
  }

  list(found = FALSE, value = "")
}

#' Read runtime configuration from .env or process env fallback
#'
#' @param name Variable name.
...
get_runtime_env_value <- function(name, unset = "") {
  dotenv_entry <- read_dotenv_entry(name)
  if (dotenv_entry$found) {
    value <- dotenv_entry$value
    return(if (nzchar(value)) value else unset)
  }
